...
class Game {
  constructor() {
    ...
    this.unlockedLevel = parseInt(localStorage.getItem("unlockedLevel")) || 0;
    this.level = null;
    this.artInfo = null;
  ...
  _bindEvents() {
    ...
    document.getElementById("menuBtn").onclick = () => {
      if (
        confirm("¿Volver al menú principal? Perderás el progreso del nivel.")
      ) {
        location.reload();
      }
    };
  ...
  _saveHighscore() {
    ...
    const currentData = {
      score: this.coins,
      time: this.initialTimer - this.timer,
    };
    ...
    if (!existing) {
      ...
      if (
        currentData.score > best.score ||
        (currentData.score === best.score && currentData.time < best.time)
      ) {
        localStorage.setItem(key, JSON.stringify(currentData));
      }
    }
        ...
        if (enemy.alive && this.physics.checkOverlap(this.player, enemy)) {
          ...
          if (isStomping) {
            ...
            if (enemy.holdsPiece !== null) {
              this.level.puzzlePieces.push(
                new PuzzlePiece(enemy.x, enemy.y, enemy.holdsPiece),
              );
            }
  ...
  completeLevel() {
    ...
    const allIndices = [
      ...this.level.defaultPieceIndices,
      ...this.level.collectedPieceIndices,
    ];
  ...
  nextLevel() {
    ...
    if (this.currentLevelIdx >= this.unlockedLevel) {

        this.unlockedLevel = this.currentLevelIdx + 1;
    ...
    this.hud.showMessage(
      title,
      isLast
        ? "Has conquistado todos los mundos."
        : `Has superado el mundo ${this.currentLevelIdx + 1}`,
      btnLabel,
      () => {
        if (isLast) {
          location.reload();
  ...
  draw() {
    ...
    this.level.coins.forEach((coin) =>
      coin.draw(this.ctx, this.camera, this.level.theme),
    );
